...
# It requires a .pdb file for the protein, a .gro file for the ligand and a preformed equilibrated
# lipid bilayer (.gro file).
...
ydim=$(cat protein.gro | awk -F ' ' '{print $2}' | awk 'END{print}')
...
lines=$((lines-3))
sed -i "2s/.*/$lines/" protein_lig.gro
sed -i "1 s/^.*/Protein with Ligand/" protein_lig.gro
sed -i "$ s/^.*/$xdim $ydim $zdim/" protein_lig.gro


xdim=$(cat popc.gro | awk -F ' ' '{print $1}' | awk 'END{print}')
ydim=$(cat popc.gro | awk -F ' ' '{print $2}' | awk 'END{print}')
zdim=$(cat protein_lig.gro | awk -F ' ' '{print $3}' | awk 'END{print}')
echo $xdim $ydim $zdim
...
cat protein_lig_newbox.gro.tmp popc.gro.tmp > protein_lig_popc.gro

lines=$(wc -l < protein_lig_popc.gro)
lines=$((lines-3))
sed -i "2s/.*/$lines/" protein_lig_popc.gro
sed -i "1 s/^.*/Protein with Ligand in POPC/" protein_lig_popc.gro
sed -i "$ s/^.*/$xdim $ydim $zdim/" protein_lig_popc.gro

# Make .ndx file of protein_lipid system
echo "q" | gmx make_ndx -f protein_lig_popc.gro
